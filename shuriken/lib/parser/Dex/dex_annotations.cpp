...
void AnnotationDirectoryItem::parse_annotation_directory_item(shurikenapi::IShurikenStream* stream) {
    ...
    stream->read_data(&class_annotations_off, sizeof(std::uint32_t));
    // then read the sizes
    stream->read_data(&fields_size, sizeof(std::uint32_t));
    stream->read_data(&annotated_methods_size, sizeof(std::uint32_t));
    stream->read_data(&annotated_parameters_size, sizeof(std::uint32_t));

    for (I = 0; I < fields_size; ++I) {
        stream->read_data(&idx, sizeof(std::uint32_t));
        stream->read_data(&annotations_off, sizeof(std::uint32_t));
