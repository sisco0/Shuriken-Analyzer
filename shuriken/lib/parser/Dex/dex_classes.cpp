...
void ClassDataItem::parse_class_data_item(shurikenapi::IShurikenStream* stream,
                                          DexFields& fields,
                                          DexMethods& methods,
                                          DexTypes& types) {
...
void ClassDef::parse_class_def(shurikenapi::IShurikenStream* stream,
                               DexStrings& strings,
                               DexTypes& types,
                               DexFields& fields,
                               DexMethods& methods) {
    ...
    stream->read_data(&classdefstruct, sizeof(classdefstruct_t));
    ...
    if (classdefstruct.interfaces_off) {
        stream->seek(classdefstruct.interfaces_off);

        stream->read_data(&size, sizeof(std::uint32_t));

        for (I = 0; I < size; ++I) {
            stream->read_data(&idx, sizeof(std::uint16_t));
    ...
    if (classdefstruct.static_values_off) {
        stream->seek(classdefstruct.static_values_off);
...
void DexClasses::parse_classes(shurikenapi::IShurikenStream* stream,
    ...
    for (I = 0; I < number_of_classes; ++I) {
        ...
        stream->seek(cur + sizeof(ClassDef::classdefstruct_t));
    }
