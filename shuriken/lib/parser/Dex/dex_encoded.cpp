...
void EncodedArray::parse_encoded_array(shurikenapi::IShurikenStream* stream,
...
void EncodedAnnotation::parse_encoded_annotation(shurikenapi::IShurikenStream* stream,
                                                 DexTypes & types,
                                                 DexStrings & strings) {
...
void EncodedValue::parse_encoded_value(shurikenapi::IShurikenStream* stream,
                                       DexTypes & types,
                                       DexStrings & strings) {
    auto read_from_stream = [&](size_t size) {
        ...
        for(size_t I = 0; I <= size; ++I) {
            stream->read_data(&aux, sizeof(std::uint8_t));
...
void EncodedCatchHandler::parse_encoded_catch_handler(shurikenapi::IShurikenStream* stream,
                                                      DexTypes& types) {
...
void CodeItemStruct::parse_code_item_struct(shurikenapi::IShurikenStream* stream,
                                            DexTypes& types) {
    ...
    stream->read_data(&code_item, sizeof(code_item_struct_t));
    ...
    for (I = 0; I < code_item.insns_size; ++I) {
        ...
        stream->read_data(&instruction, sizeof(std::uint8_t[2]));
    ...
    if ((code_item.tries_size > 0) && // padding present in case tries_size > 0
        ...
        stream->seek(cur + sizeof(std::uint16_t));
    }
    ...
    if (code_item.tries_size > 0) {
        ...
        for (I = 0; I < code_item.tries_size; ++I) {
            stream->read_data(&try_item, sizeof(TryItem));
...
void EncodedMethod::parse_encoded_method(shurikenapi::IShurikenStream* stream,
